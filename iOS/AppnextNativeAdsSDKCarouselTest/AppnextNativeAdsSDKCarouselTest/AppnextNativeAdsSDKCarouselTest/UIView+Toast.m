...
- (void)hideToastInternal:(UIView *)toast;
...
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)interval title:(NSString *)title {
    [self makeToast:message duration:interval position:CSToastDefaultPosition title:title];
}
...
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration point:(CGPoint)position {
    UIView *toast = [self viewForMessage:message title:nil image:nil];
    [self showToast:toast duration:duration point:position];
}

- (void)makeToastWithinContainigView:(NSString *)message duration:(NSTimeInterval)duration point:(CGPoint)position {
    UIView *toast = [self viewForMessage:message title:nil image:nil];
    [self showToastWithinContainigView:toast duration:duration point:position];
}
...
- (void)showToast:(UIView *)toast duration:(NSTimeInterval)duration point:(CGPoint)point{
    toast.center = point;
    toast.alpha = 0.0;
...
- (void)showToastWithinContainigView:(UIView *)toast duration:(NSTimeInterval)duration point:(CGPoint)point
{
    UIView * dummy = [[UIView alloc]initWithFrame:toast.frame];
    dummy.center = point;
    
    // ensure the toast doesn't get out of it's containing view boundaries:
    if(dummy.frame.origin.x < self.frame.origin.x)
    {
        dummy.center = CGPointMake(point.x + (self.frame.origin.x - dummy.frame.origin.x), point.y);
    }
    if((dummy.frame.origin.x + dummy.frame.size.width) > (self.frame.size.width))
    {
        dummy.center = CGPointMake(point.x - ((dummy.frame.origin.x + dummy.frame.size.width) - (self.frame.size.width)) - 10, point.y);
    }

    [self showToast:toast duration:duration point:dummy.center];
...
- (void)toastTimerDidFinish:(NSTimer *)timer {
    [self hideToastInternal:(UIView *)timer.userInfo];
}
